...
bool CAttemperEngineSink::OnDBLogonSuccess(void * pDataBuffer, WORD wDataSize, NTY_DataBaseEvent * pDataBaseEvent)
{
	...
	pCMDLogonSuccess->dwTZPoint=pDBRLogonSuccess->dwTZPoint;
	...
	SendPacket.AddPacket(pDBRLogonSuccess->szName,CountString(pDBRLogonSuccess->szName),DTP_USER_NAME);
	SendPacket.AddPacket(pDBRLogonSuccess->szPassWord,CountString(pDBRLogonSuccess->szPassWord),DTP_USER_PASS);
	SendPacket.AddPacket(pDBRLogonSuccess->szFace,CountString(pDBRLogonSuccess->szFace),DTP_USER_FACE);
	//站点主页
	...
	case SUB_GP_REGISTER_ACCOUNTS:		//注册帐号
		{
			...
			lstrcpyn(RegisterAccounts.szFace,pRegisterAccounts->szFace,sizeof(RegisterAccounts.szFace));

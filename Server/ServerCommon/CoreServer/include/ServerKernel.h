...
//ADO 导入库
#import "../Depend/MSADO15.DLL" rename_namespace("ADOCG") rename("EOF","EndOfFile")
...
//模块定义
#ifdef _DEBUG
	#define CORESERVERL_DLL_NAME	TEXT("CoreServer_D.dll")			//组件 DLL 名字
#else
	#define CORESERVERL_DLL_NAME	TEXT("CoreServer.dll")			//组件 DLL 名字
#endif
...
interface IDataBase : public IUnknownEx
{
	...
	//设置信息
	virtual bool __cdecl SetConnectionInfo(LPCTSTR szIP, LPCTSTR szData, LPCTSTR szName, LPCTSTR szPass)=NULL;
	//是否连接错误

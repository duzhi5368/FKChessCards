...
BEGIN
	...
	SELECT @UserID=UserID, @Accounts=Accounts, @Name=[Name], @LogonPass=LogonPass, @Gender=Gender, @LogonNullity=LogonNullity, 
		@UserRight=UserRight, @MasterRight=MasterRight, @Face=Face, @MemberOrder=MemberOrder, 
		@MemberOverDate=MemberOverDate, @TZPoint=TZPoint 
	FROM TZUserDBServer.TZUserDB.dbo.UserAccounts WHERE UserID=@dwUserID
	...
	BEGIN
		SET @ErrorDescribe='你已经在充值游戏房间了，不能同时在进入此游戏房间了！'
		SELECT [ErrorDescribe]=@ErrorDescribe
		RETURN 2
	END
	...
	BEGIN
		...
		-- 读取游戏信息
		SELECT @Score=Score, @TZPoint=TZPoint, @WinCount=WinCount, @LostCount=LostCount, @DrawCount=DrawCount, @DrawCount=DrawCount, @FleeCount=FleeCount, 
			   @GameUserRight=UserRight, @GameMasterRight=MasterRight FROM GameScore WHERE UserID=@dwUserID
	END
	...
	-- 输出变量
	SELECT @UserID AS UserID, @Accounts AS Accounts, @Name AS [Name], @Face AS Face, @Gender AS Gender, 
		@UserRight|@GameUserRight AS UserRight, @MasterRight|@GameMasterRight AS MasterRight, @TZPoint AS TZPoint, 
		@MemberOrder AS MemberOrder, @Score AS Score,

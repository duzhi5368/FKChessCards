...
int getNextNal(char* inpf, int size, unsigned char* Buf)
{
	int pos = 0;
	int pos1 = 0;
	...
	while(pos1<size && (Buf[pos++]=inpf[pos1++])==0);
...
//AE by tiany for test on 20060403

//#if 0 //selftest
#define COUNT 8000
#define SIZE (COUNT*40)
extern int Decode( char* Source, int size, char* OutData ){
//    int i;
/*    uint8_t temp[SIZE];
    PutBitContext pb;
    GetBitContext gb;
...
//AS by tiany for test on 20060331
	...
	nalLen = 0;
	Buf = (unsigned char*)calloc ( 0x10000, sizeof(char));
   ...
   if (avcodec_open(c, codec) < 0) {
	...
	{
		nalLen = getNextNal(Source+sizeindex, size-sizeindex, Buf);
		sizeindex += nalLen;
...
//	       c->frame_number++;
		...
		if(consumed_bytes > 0)
		{
			...
			{
				memcpy( OutData+OutIndex, picture->data[0] + i * picture->linesize[0], c->width );
				OutIndex += c->width;
				//fwrite(picture->data[0] + i * picture->linesize[0], 1, c->width, outf);
			}				
			for(i=0; i<c->height/2; i++)
			{
				memcpy( OutData+OutIndex, picture->data[1] + i * picture->linesize[1], c->width/2 );
				OutIndex += c->width/2;
				//fwrite(picture->data[1] + i * picture->linesize[1], 1, c->width/2, outf);
			}
			for(i=0; i<c->height/2; i++)
			{
				memcpy( OutData+OutIndex, picture->data[2] + i * picture->linesize[2], c->width/2 );
				OutIndex += c->width/2;

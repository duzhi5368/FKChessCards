...
#include "../include/UserProcess.h"
	...
	#pragma comment( lib, "Depend\\DXBaseClasses\\Bin\\DXBaseClasses_D.lib" )
#else
	#pragma comment( lib, "Depend\\DXBaseClasses\\Bin\\DXBaseClasses.lib" )
#endif
...
void CVideoClient::OutVideo( DWORD Pos, char* pData, int len )
{
	if( Pos == INFINITE )
	{
		if( CUserProcess::m_ServerIP.POS!=0 )
		{
			...
			//m_VideoWindow[1].OutVideo( pData, len );
		}
	}
	else
...
void CVideoClient::Setup()
{
	InMin.Setup();
}

void CVideoClient::Enable( bool b )
{
	m_Enable = b;
	if( b )
	{
